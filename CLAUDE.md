# Claude Code Project Instructions — opentelemetry-sdk-kotlin

## Project Overview

This is **opentelemetry-sdk-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`opentelemetry_sdk`](https://crates.io/crates/opentelemetry_sdk).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/opentelemetry_sdk/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/opentelemetry_sdk>
